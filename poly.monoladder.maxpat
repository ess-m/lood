...
, 			{
				"box" : 				{
					...
					"numoutlets" : 1,
					"outlettype" : [ "" ],
					"patching_rect" : [ 945.0, 192.0, 107.0, 22.0 ],
					"text" : "scale 0. 1. -48. 48."
				}
...
, 			{
				"box" : 				{
					...
					"numoutlets" : 1,
					"outlettype" : [ "" ],
					"patching_rect" : [ 705.0, 192.0, 107.0, 22.0 ],
					"text" : "scale 0. 1. -48. 48."
				}
...
, 							{
						...
						"bgcolor" : [ 0.305882352941176, 0.305882352941176, 0.313725490196078, 1.0 ],
...
, 			{
				"box" : 				{
					...
					"outlettype" : [ "int" ],
					"parameter_enable" : 0,
					"patching_rect" : [ 396.0, 241.0, 24.0, 24.0 ]
				}
...
, 			{
				"box" : 				{
					...
					"numoutlets" : 1,
					"outlettype" : [ "signal" ],
					"patching_rect" : [ 396.0, 297.0, 31.0, 22.0 ],
					"text" : "sig~"
				}
...
, 			{
				"box" : 				{
					...
					"numoutlets" : 2,
					"outlettype" : [ "bang", "" ],
					"patching_rect" : [ 303.0, 198.0, 34.0, 22.0 ],
					"text" : "sel 0"
				}
...
, 			{
				"box" : 				{
					...
					"numoutlets" : 1,
					"outlettype" : [ "" ],
					"patching_rect" : [ 945.0, 285.0, 55.0, 22.0 ],
					"text" : "tune1 $1"
				}
...
, 			{
				"box" : 				{
					...
					"numoutlets" : 1,
					"outlettype" : [ "" ],
					"patching_rect" : [ 705.0, 285.0, 55.0, 22.0 ],
					"text" : "tune0 $1"
				}
...
, 			{
				"box" : 				{
					...
					"numoutlets" : 1,
					"outlettype" : [ "signal" ],
					"patching_rect" : [ 318.0, 297.0, 39.0, 22.0 ],
					"text" : "click~"
				}
...
, 			{
				"box" : 				{
					...
					"numoutlets" : 1,
					"outlettype" : [ "bang" ],
					"patching_rect" : [ 318.0, 241.0, 22.0, 22.0 ],
					"text" : "t b"
				}
...
, 							{
								"box" : 								{
									"code" : "// attack-release envelope \r\n\r\nar(trig, gate, attack, release) {\n\tHistory env(0), stage(0),\n\t\t\tatk_rate(0), atk_coeff(0), atk_offset(0),\n\t\t\trel_rate(0), rel_coeff(0), rel_offset(0);\n\t\t\r\n\tif (trig) {\r\n\t\t// rate in seconds\n\t\tatk_rate = samplerate * attack;\n\t\trel_rate = samplerate * release;\n\t\t\n\t\t// slope 0 = linear, slope -n = log/exp\n\t\trel_slope = exp(-8);\n\t\t\n\t\t// calculate coefficients\n\t\tatk_offset = 1 / atk_rate;\n\t\t\n\t\trel_coeff = exp(-log((1 + rel_slope) / rel_slope) / rel_rate);\n\t\trel_offset = -rel_slope * (1 - rel_coeff);\n\t\t\n\t\t// reset & trigger envelope\n\t\tstage = 1;\n\t}\t\n\n\t// stage 0 = idle, 1 = attack, 2 = decay\n\tif (stage == 0) {\n\t\tenv = 0;\n\t} else if (stage == 1) {\n\t\tenv = atk_offset + env;\r\n\t\t\r\n\t\tif (!gate) {\r\n\t\t\tstage = 2;\r\n\t\t}\n\t\t\n\t\tif (env >= 1 || attack <= 0) {\n\t\t\tenv = 1;\n\t\t\tstage = 2;\n\t\t}\r\n\t} else {\n\t\tenv = rel_offset + env * rel_coeff;\n\t\t\n\t\tif (env <= 0 || release <= 0) {\n\t\t\tenv = 0;\n\t\t\tstage = 0;\n\t\t}\r\n\t}\n\treturn env;\n}\r\n\r\n// attack-hold-release envelope\r\n\r\nahr(trig, gate, attack, release) {\n\tHistory env(0), stage(0), trigged(0), gating(0),\n\t\t\tatk_rate(0), atk_coeff(0), atk_offset(0),\n\t\t\trel_rate(0), rel_coeff(0), rel_offset(0);\n\t\t\r\n\tif (trig) {\r\n\t\tif (gate && trigged) {\r\n\t\t\t// do nothing\r\n\t\t} else {\r\n\t\t\ttrigged = 1;\r\n\t\t\t\r\n\t\t\t// rate in seconds\n\t\t\tatk_rate = samplerate * attack;\n\t\t\trel_rate = samplerate * release;\n\t\t\n\t\t\t// slope 0 = linear, slope -n = log/exp\n\t\t\trel_slope = exp(-8);\n\t\t\n\t\t\t// calculate coefficients\r\n\t\t\tatk_offset = 1 / atk_rate;\n\t\t\n\t\t\trel_coeff = exp(-log((1 + rel_slope) / rel_slope) / rel_rate);\n\t\t\trel_offset = -rel_slope * (1 - rel_coeff);\n\t\t\n\t\t\t// reset & trigger envelope\n\t\t\tstage = 1;\r\n\t\t}\n\t}\t\n\n\t// stage 0 = idle, 1 = attack, 2 = decay\n\tif (stage == 0) {\n\t\tenv = 0;\n\t} else if (gate && trigged) {\r\n \t\tif (stage == 1) {\n\t\t\tenv = atk_offset + env;\n\t\t\n\t\t\tif (env >= 1 || attack <= 0) {\n\t\t\t\tenv = 1;\n\t\t\t\tstage = 2;\n\t\t\t}\r\n\t\t} else {\n\t\t\tenv = env;\n\t\t}\r\n\t} else {\r\n\t\ttrigged = 0;\r\n\t\t\n\t\tenv = rel_offset + env * rel_coeff;\n\t\t\n\t\tif (env <= 0 || release <= 0) {\n\t\t\tenv = 0;\n\t\t\tstage = 0;\n\t\t}\r\n\t} \n\treturn env, trigged;\n}\r\n\r\n// sine approximation\r\n\r\nsin_approx(x) {\r\n\tx_ = phasewrap(x);\r\n\tx2 = x_ * x_;\r\n\t\r\n\ty0 = (x2 * -2.39e-08 + 2.7526e-06) * x2 + -0.000198;\r\n\ty1 = ((x2 * y0 + 0.008333) * x2 + -0.166667) * x2 + 1;\r\n\t\r\n\treturn x_ * y1;\r\n}\r\n\r\n// leaky integrator\r\n\r\nintegrator_single(x, y) {\r\n\tHistory z0(0), z1(0);\r\n\t\r\n\ty_ = y * 0.25;\r\n\tx_ = ((x - z0) * y_ * 0.25) + z0;\r\n\tz0 = x_;\r\n\t\r\n\tx2 = (x_ - z1) + z1;\r\n\tz1 = x2;\r\n\t\r\n\treturn z1 * 2.5;\r\n}\r\n\r\nintegrator_double(x, y) {\r\n\tHistory z0(0), z1(0), z2(0);\r\n\t\r\n\ty_ = y * 0.25;\r\n\tx_ = ((x - z0) * y_ * 0.25) + z0;\r\n\tz0 = x_;\r\n\t\r\n\tx2 = (x_ - z1) + z1;\r\n\tz1 = x2;\r\n\t\r\n\tx3 = ((x_ - z2) * y_) + z2;\r\n\tz2 = x3;\r\n\t\r\n\treturn z1 * 2.25, z2 * 7;\r\n}\r\n\r\n// band limited impulse train\r\n\r\nblit(freq, shape) {\r\n\tHistory phase(0);\r\n\t\r\n\tfreq_accum = (freq * twopi) / samplerate;\r\n\tfreq_limit = (samplerate / 2) / freq;\r\n\tfreq_floor = floor(freq_limit);\r\n\tfreq_mix = freq_limit - freq_floor;\r\n\t\r\n\tphase = phasewrap(phase + freq_accum);\r\n\tphase_offset = phasewrap(phase + pi * (1 - (clip(shape, 0.5, 1) - 0.5) * 1.95));\r\n\t\r\n\tf0 = (freq_floor - 1) + 0.5;\r\n\tf1 = freq_floor + 0.5;\r\n\t\r\n\tk0 = sin_approx(f0 * phase_offset);\r\n\tk1 = 1 / sin_approx(phase_offset * 0.5);\r\n\tk2 = sin_approx(f1 * phase_offset);\r\n\t\r\n\tk3 = sin_approx(f0 * phase);\r\n\tk4 = 1 / sin_approx(phase * 0.5);\r\n\tk5 = sin_approx(f1 * phase);\r\n\t\r\n\tmix_factor = freq_limit - freq_floor;\r\n\t\r\n\tm0 = mix(k0 * k1 - 1, k2 * k1 - 1, mix_factor);\r\n\tm1 = mix(k3 * k4 - 1, k5 * k4 - 1, mix_factor);\r\n\t\r\n\tt0 = latch(m0, phase_offset);\r\n\tt1 = latch(m1, phase);\r\n\t\r\n\tpul, tri = integrator_double(t0 - t1, freq_accum);\r\n\tsaw = integrator_single(t1, freq_accum);\r\n\t\r\n\tosc_out = mix(tri, -saw, min(shape, 0.25) * 4);\r\n\tosc_out = mix(osc_out, pul, (clip(shape, 0.25, 0.5) - 0.25) * 4);\r\n\t\r\n\treturn osc_out;\r\n}\r\n\r\n// onepole smoother for event rate signals\r\n\r\nsmooth(x, time) { \n    History prev;\n    prev = prev + time * (x - prev);        \n\n    return prev;\n}\r\n\r\nParam \tcut, res, fil_dep,\r\n\t\tshape, sub, detune, slide, tune0, tune1,\r\n\t\tatk, dec, fdec;\r\n\r\nHistory slide_time;\r\n\r\n// inputs\r\n\r\nnote = in1;\r\ntrig = in2;\r\ngate = in3;\r\nbend = in4;\r\n\r\n// parameters\r\n\r\ncut_ = smooth(cut * 127, 0.005);\r\nres_ = smooth(res * 24 + 1, 0.005);\r\nshape_ = smooth(shape, 0.005);\r\n\r\nfdec_uni = fdec * 2 - 1;\r\nfatk = abs(clip(fdec_uni, -1, 0)) * 8;\r\nfatk = smooth(fatk + 0.002, 0.005);\r\n\r\ndec_ = smooth(dec * 16 + 0.01, 0.005);\r\nfdec_ = smooth(abs(fdec_uni) * 32 + 0.01, 0.005);\r\n\r\nshape_mix = clip(shape_, 0, 0.5) * 2;\r\npulse_width = (clip(shape_, 0.5, 1) - 0.5) * 2;\r\n\r\ntune0_mix = smooth(clip(abs(tune0), 0, 0.0001) * 10000, 0.0001);\r\ntune1_mix = smooth(clip(abs(tune1), 0, 0.0001) * 10000, 0.0001);\r\n\r\n// envelopes\r\n\r\namp_env, trig_state = ahr(trig, gate, 0.002, dec_);\r\nfil_env = ar(trig, gate, fatk, fdec_);\r\n\r\n// frequency calculations\r\n\r\nnote = note + (bend * 12);\r\n\r\nlast_note = latch(note, delta(trig_state) > 0);\r\nslide_ms = mstosamps(24);\r\n\r\nif (trig_state) {\r\n\tif (note != last_note) {\r\n\t\tslide_time = slide_ms;\r\n\t}\r\n} else {\r\n\tslide_time = 0;\r\n}\r\n\r\nnote_ = slide(note, slide_time, slide_time);\r\n\r\nfreq = mtof(note_);\r\n\r\nfil_dep_ = smooth(fil_dep * 127, 0.005);\r\n\r\ncutoff = mtof(smooth(cut_, 0.005) + (note_ - 24) + fil_env * fil_dep_);\r\ncutoff = clip(cutoff, 1, samplerate / 2 - 500);\r\n\r\ndet0 = smooth(tune0, 0.005);\r\ndet1 = smooth(tune1, 0.005);\r\n\r\nosc_0 = blit(freq, shape_);\r\nosc_1 = blit(mtof(note_ + det0), shape_) * tune0_mix;\r\nosc_2 = blit(mtof(note_ + det1), shape_) * tune1_mix;\r\n\r\nout1 = (osc_0 + osc_1 + osc_2) * 0.4;\r\nout2 = amp_env;\r\nout3 = cutoff;\r\nout4 = res_;",
									"fontface" : 0,
									"fontname" : "<Monospaced>",
									"fontsize" : 12.0,
									"id" : "obj-5",
